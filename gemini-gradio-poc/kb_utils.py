...
def core_build_knowledge_base(file_paths: List[str], chunk_size: int, chunk_overlap: int) -> Tuple[str, pd.DataFrame]:
    ...
    if not raw_docs:
        return "No readable documents found.", pd.DataFrame()
    all_chunks = []
    all_filenames = []
    for doc in raw_docs:
    ...
    if not all_chunks:
        return "No text chunks created from documents.", pd.DataFrame()
    try:
        ...
        if not successful_pairs:
            return "Embedding failed for all chunks.", pd.DataFrame()
        ...
        if len(filtered_chunks_aligned) != len(successful_embeddings):
            return "Internal error aligning chunks/embeddings.", pd.DataFrame()
    except Exception as e:
        return f"An error occurred during embedding: {e}", pd.DataFrame()
    try:
        ...
        return f"An error occurred creating the index: {e}", pd.DataFrame()
